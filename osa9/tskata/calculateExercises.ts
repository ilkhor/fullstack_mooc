...
export const calculateExercises = (target: number, exercises: Array<number>): ExerciseResult => {

    const avg = exercises.reduce( (a: number, b: number) => a + b, 0) / exercises.length;
    ...
    return {
        ...
        target: target,
        average: exercises.reduce((a: number, b: number) => a + b, 0) / exercises.length
    };
};
